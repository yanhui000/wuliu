...
var getAppEnvInfo = function () {
  var userAgent = window._YMMUA || window.navigator.userAgent
  var str = userAgent.match(/ymm56\/.*/gi)
  var data = {
  }
  if (str && str[ 0 ]) {
    str = str[ 0 ].split(' ')[0].trim()
    str = str.split('/')
    data.network = str[ 2 ]
    if (str[ 1 ]) {
      data.appName = str[ 1 ].split(':')[ 0 ] || ''
      data.appName = data.appName.toLowerCase()
      // consignor shipper 货主2种标记
      if (data.appName == 'consignor') {
        data.appName = 'shipper'
      }
      data.appType = 0 // 老的客户端类型字段，与服务端定义相反了（1：货主端、2：司机端）
      data.clientType = 0 // 新的客户端字段，与服务端、客户端定义相同（1：司机端、2：货主端）
      if (data.appName == 'driver') {
        data.appType = 2
        data.clientType = 1
      } else if (data.appName == 'shipper') {
        data.appType = 1
        data.clientType = 2
      }
      data.version = str[ 1 ].split(':')[ 1 ]
    }
    // jsBridge 版本信息
    data.jsBridge = (str[ 3 ] || 1) - 0
  }
  return data
}
console.log(getAppEnvInfo())
Raven&&Raven.setTagsContext(getAppEnvInfo())