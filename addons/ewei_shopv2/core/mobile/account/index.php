...
class Index_EweiShopV2Page extends MobilePage
{
	...
	public function login_app()
	{
		...
		if ($_W['ispost']) {
			$mobile = trim($_GPC['mobile']);
			$pwd = trim($_GPC['pwd']);
			$member = pdo_fetch('select id,openid,mobile,pwd,salt,nickname from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
	...
	public function verifycode()
	{
		...
		if ($temp == 'sms_reg' && !empty($member)) {
			pdo_update('ewei_shop_member', array('pwd'=>md5($code . $member['salt'])), array('id' => $member['id']));
		}
		if(empty($member)){
			if (empty($openid)) {
					$openid = 'wap_user_' . $_W['uniacid'] . '_' . $mobile;
			...
			$salt = m('account')->getSalt();
			$data1 = array('uniacid' => $_W['uniacid'], 'mobile' => $mobile, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($code . $salt), 'salt' => $salt, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile');
