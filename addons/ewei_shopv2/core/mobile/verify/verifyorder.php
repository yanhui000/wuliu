...
class Verifyorder_EweiShopV2Page extends MobilePage
{
	public function orderData()
	{
		...
		if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)) {
			...
			$sql = 'select * ,0 as isverifygoods_order  from ' . tablename('ewei_shop_order') . ' where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0  GROUP BY id ORDER BY createtime DESC  ';
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		if ($day != 0) {
			$createtime1 = strtotime(date('Y-m-d', strtotime('-7 days')));
		...
		$sql = 'SELECT count(o.id) FROM' . tablename('ewei_shop_order') . ' o inner JOIN ' . tablename('ewei_shop_saler') . " s ON o.verifyopenid=s.openid   WHERE \r\n        s.openid=:openid and o.createtime between :createtime1 and :createtime2 AND o.ismr=0  AND   o.status =3  AND o.deleted=0 and o.uniacid=:uniacid   limit 1";
