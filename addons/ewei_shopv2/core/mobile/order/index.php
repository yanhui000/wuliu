...
class Index_EweiShopV2Page extends MobileLoginPage
{
	...
	public function get_list()
	{
		...
		foreach ($list as $k=>$v){
			if(!empty($v['merchid'])){
				$merch1 = pdo_fetch('select * from ' . tablename('ewei_shop_merch_user') . ' where id=:id', array(':id' => $v['merchid']));
				$list[$k]['img']=$merch1['img'];
				$list[$k]['chepai']=$merch1['chepai'];
				$list[$k]['mobile']=$merch1['mobile'];
				$list[$k]['realname']=$merch1['realname'];
			}
		}
//var_dump($list);die;
		unset($row);
	...
	public function detail()
	{
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			if(!empty($order['merchid'])){
				$merch1 = pdo_fetch('select * from ' . tablename('ewei_shop_merch_user') . ' where id=:id', array(':id' => $order['merchid']));
				$order['img']=$merch1['img'];
				$order['chepai']=$merch1['chepai'];
				$order['mobile']=$merch1['mobile'];
				$order['realname']=$merch1['realname'];
			}
