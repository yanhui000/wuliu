...
<<<<<<< HEAD
			...
			header('location: ' . webUrl('sysset/soso'));
		}else if (cv('sysset.update ')){
			header('location: ' . webUrl('sysset/update'));
		}else {
	...
	public function freightset()
	{
		...
		if ($_GPC['enabled'] != '' && $_GPC['keyword'] != ''){
			...
			$addressInfo = pdo_fetchall("SELECT * FROM ".tablename('cargo_freight'));
		}
		if (!empty($addressInfo)) {
			//处理 省市区
			foreach ($addressInfo as $k => $v) {
				$addressInfo[$k]['f_origin_pro'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_origin_pro']));
				$addressInfo[$k]['f_origin_city'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_origin_city']));
				$addressInfo[$k]['f_origin_county'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_origin_county']));
				$addressInfo[$k]['f_des_pro'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_des_pro']));
				$addressInfo[$k]['f_des_city'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_des_city']));
				$addressInfo[$k]['f_des_county'] = pdo_fetchall("SELECT name FROM" . tablename('area') . "where id = :uid" , array(':uid' => $v['f_des_county']));
			}
		}else {
			$addressInfo = [];
		}
//		var_dump($addressInfo);
		include $this->template('sysset/freightset');
	}
	
	//添加路程运费
	public function freightsetadd()
	{
		include $this->template('sysset/freightsetadd');
	}
	//查询地理位置
	public function freightsetadd2(){
		$type = isset($_GET['type'])?$_GET['type']:0;//获取请求信息类型 1省 2市 3区
		$province_num = isset($_GET['pnum'])?$_GET['pnum']:'110000';//根据省编号查市信息
		$city_num = isset($_GET['cnum'])?$_GET['cnum']:'110001';//根据市编号查区信息
		switch ($type) {//根据请求信息类型，组装对应的sql
			case 1://省
				$data = pdo_getall('area',array('pid ==' => 0));
				break;
			case 2://市
				$data = pdo_getall('area',array('pid ==' => $province_num));
				break;
			case 3://区
				$data = pdo_getall('area',array('pid ==' => $city_num));
				break;
			default:
				die('no data');
				break;
	...
	//修改查询地理位置数据库
	public function freightsetadd3(){
		$result = pdo_getall('area');
		show_json(0, $result);
	}
	//添加数据库
	public function adddata()
	{
		...
		global $_GPC;
		if ($_W['ispost']){
			$f_origin_city = $_GPC['data']['f_origin_city'];
			$f_des_city = $_GPC['data']['f_des_city'];
			$f_origin_county = $_GPC['data']['f_origin_county'];
			$f_des_county = $_GPC['data']['f_des_county'];
			if ($f_origin_county == 0 || $f_des_county == 0){
				$id = pdo_get('cargo_freight', array('f_origin_city' => $f_origin_city, 'f_des_city' => $f_des_city), array('f_id'));
				if (!$id) {
					$id = pdo_get('cargo_freight', array('f_origin_city' => $f_des_city, 'f_des_city' => $f_origin_city), array('f_id'));
				}
			}else {
				$id = pdo_get('cargo_freight', array('f_origin_county' => $f_origin_county, 'f_des_county' => $f_des_county), array('f_id'));
				if (!$id) {
					$id = pdo_get('cargo_freight', array('f_origin_county' => $f_des_county, 'f_des_county' => $f_origin_county), array('f_id'));
				}
			}
			if (!$id){
				$_GPC['data']['f_add_time'] = time();
				array_shift($_GPC['data']);
				unset($_GPC['data']['strategy']);
				$res = pdo_insert('cargo_freight',$_GPC['data']);
				show_json(0);
			}
			show_json(1);
	...
	public function del(){
		...
		if ($_W['ispost']){
			$id = $_GPC['id'];
			$data = pdo_getall('cargo_freight',array('f_id ' => $id));
			$where = ['f_id' => $id];
			if ($data) {
				$res = pdo_delete('cargo_freight',array('f_id' => $id));
				show_json(0);
			}
		}
	}
	...
	public function update(){
		...
		$f_id = $_GPC['id'];
		//查询要修改的数据
		$addressInfo = pdo_get('cargo_freight',array('f_id ' => $f_id));
		//查询所有的省
		$provinceInfo = pdo_getall('area',array('pid ' => 0));
		//查询所有的市起始
		$cityInfo = pdo_getall('area',array('pid ' => $addressInfo['f_origin_pro']));
		//查询所有的县
		$areaInfo = pdo_getall('area',array('pid ' => $addressInfo['f_origin_city']));

		//查询所有的市终点
		$des_cityInfo = pdo_getall('area',array('pid ' => $addressInfo['f_des_pro']));
		//查询所有的县
		$des_areaInfo = pdo_getall('area',array('pid ' => $addressInfo['f_des_city']));
		include $this->template('sysset/update');
	}
	...
	public function updateadd(){
		...
		global $_GPC;
		if ($_W['ispost']){
			$res = pdo_insert('cargo_freight',$_GPC['data'], true);
			if ($res) {
				show_json(0);
			} else {
				show_json(1);
