...
class Credit_EweiShopV2Page extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		if ($type == 'credit1') {
			$sql = 'select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . ' left join ' . tablename('ewei_shop_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('ewei_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ewei_shop_member_level') . ' l on m.level =l.id' . (' where 1 ' . $condition . ' ORDER BY log.createtime DESC ');
		}
		else {
			if ($type == 'credit2') {
				...
				$table2 = 'select log.id,log.money,log.createtime,log.remark,\'credit2\' as credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,\'admin\' as username,g.groupname,l.levelname from ' . tablename('ewei_shop_member_log') . 'as log ' . ' inner join ' . tablename('ewei_shop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('ewei_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ewei_shop_member_level') . ' l on m.level =l.id' . (' where m.uid=0 and log.status=1 ' . $condition1);
