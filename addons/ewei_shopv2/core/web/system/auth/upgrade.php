...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		if(!$auth['code'])show_json(0, '目前程序未授权一键更新功能，请先授权然后更新！<a style="color:red;" href="index.php?c=site&a=entry&m=ewei_shopv2&do=web&r=system.auth">点我进入授权界面，填写授权码</a>');
		...
		$resp = ihttp_request(HTTP_X_FOR .'vip.rnds168.cn/app/ewei.php?a=check', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request(HTTP_X_FOR .'vip.rnds168.cn/app/ewei.php?a=download', array('ip' => $auth['ip'], 'modname' => $auth['modname'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
	...
	protected function updateComplete($version, $release)
	{
		...
		$sql ="UPDATE  `ims_modules` SET  `version` =  '$version',`author` =  '美仑科技',`url` =  'http://www.weby.cc' WHERE  `name` ='ewei_shopv2'";
		pdo_query($sql);
