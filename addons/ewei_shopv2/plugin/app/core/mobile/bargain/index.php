...
class Index_EweiShopV2Page extends AppMobilePage
{
	...
	public function bargain()
	{
		...
		if (0 < $res2['time_limit']) {
			...
			$res2['end_time'] = $showtime;
		}
		else {
			$res2['start_time'] = strtotime($res['created_time']);
		...
		if ($ajax == 151) {
			...
			if ($open_redis) {
				$redis_key = $_W['uniacid'] . '_bargain_' . $_W['openid'];
				$redis = redis();
	...
	public function join()
	{
		...
		if (!empty($res['initiate'])) {
			$count = pdo_get('ewei_shop_bargain_actor', array('goods_id' => $bargain_id, 'openid' => $_W['openid'], 'status' => 0, 'orderid' => 0), 'id');
