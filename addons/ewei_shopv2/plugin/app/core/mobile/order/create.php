	...
	public function main()
	{
		...
		if (!$packageid) {
			...
			if (empty($id)) {
				...
				if (!empty($optionid)) {
					$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,`virtual`,stock,weight,specs,isfullback,`day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,cycelbuy_periodic from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
		...
		$result = array(
			...
			'storeInfo'          => !empty($store_list) ? $store_list[0] : false,
			'address'            => $address,
			'goods'              => $allgoods,
			'merchid'            => $merch_id,
			'packageid'          => $packageid,
		...
		if (0 < $packageid) {
			...
			if (is_array($goodsarr)) {
				...
				foreach ($goodsarr as &$g) {
					...
					if (!empty($optionid)) {
						$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight,cycelbuy_periodic from ' . tablename('ewei_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
			...
			if ($data['type'] == 5) {
				if ($data['verifygoodslimittype'] == 1) {
					if ($data['verifygoodslimitdate'] <= time()) {
						app_error(AppError::$GoodsNotFound, '"' . $data['title'] . '"商品使用时间已失效,无法购买 !');
					}

					if ($data['verifygoodslimitdate'] - 3600 * 3 <= time()) {
						app_error(AppError::$GoodsNotFound, '"' . $data['title'] . '"商品的使用时间即将失效,无法购买 !');
					}
					else {
						if ($data['verifygoodslimittype'] == 0) {
							...
							if ($data['verifygoodsdays'] * 3600 * 24 - 3600 * 3 <= time()) {
								app_error(AppError::$GoodsNotFound, '"' . $data['title'] . '"商品的使用时间即将失效,无法购买 !');
							}
						}
					}
