...
class Index_EweiShopV2Page extends AppMobilePage
{
	...
	public function get_list()
	{
		...
		$list = pdo_fetchall('select id,ordersn,price,userdeleted,isparent,refundstate,paytype,status,addressid,refundid,isverify,dispatchtype,verifytype,verifyinfo,verifycode,iscomment,iscycelbuy from ' . tablename('ewei_shop_order') . (' where 1 ' . $condition . ' order by createtime desc LIMIT ') . ($pindex - 1) * $psize . ',' . $psize, $params);
	...
	public function detail()
	{
		...
		if ($order['isonlyverifygoods']) {
			...
			$verifygood = pdo_fetchcolumn('select * from ' . tablename('ewei_shop_verifygoods') . ' where orderid=:orderid limit 1', array(':orderid' => $order['id']));
	...
	public function express()
	{
		...
		if (!empty($cycelid)) {
			$order = pdo_fetch('select expresscom,expresssn,addressid,status,express from ' . tablename('ewei_shop_cycelbuy_periods') . ' where id=:id and uniacid=:uniacid', array(':id' => $cycelid, ':uniacid' => $uniacid));
		}
		else {
	...
	public function cycelbuy_list()
	{
		...
		$result = array('list' => $cycelData, 'cycelUnderway' => $cycelUnderway['count'], 'cycelbuy_periodic' => $orderData['cycelbuy_periodic'], 'maxday' => $cycelbuy_set['max_day'], 'notStart' => $notStart, 'period_index' => $period_index, 'orderid' => $orderData['id']);
		app_json($result);
	}
	...
	public function address()
	{
		...
		$show_data = 1;
		$applyforid = intval($_GPC['cycelid']);
		$orderid = intval($_GPC['id']);
		$sql = 'SELECT id,cycelbuy_periodic FROM ' . tablename('ewei_shop_order') . ' WHERE id=:orderid AND uniacid =:uniacid ';
		$orderData = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		...
		if (!empty($applyforid)) {
			$address = pdo_get('ewei_shop_address_applyfor', array('id' => $applyforid, 'isdelete' => 0));
