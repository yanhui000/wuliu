...
class LiveModel extends PluginModel
{
	...
	public function getLiveInfo($url = NULL, $type = 'auto')
	{
		...
		case 'panda':
			...
			$apiother = ihttp_get('https://api.m.panda.tv/stream/room/pull/get?roomid=' . $roomid . '&roomkey=' . $apiResult['data']['videoinfo']['room_key'] . '&definition_option=1&hardware=1');
			$apiotherResult = json_decode($apiother['content'], true);
			$sign = $apiotherResult['data'][$roomid]['sign'];
			$ts = $apiotherResult['data'][$roomid]['ts'];
