...
class Batch_EweiShopV2Page extends PluginWebPage
{
	...
	public function getesheetTemp()
	{
		...
		if ($_W['ispost']) {
			...
			$template = pdo_fetchcolumn('SELECT print_template  FROM ' . tablename('ewei_shop_order') . ' WHERE id=:id limit 1', array('id' => $orderids[0]));
			if (empty($_GPC['printarr']['ordercode']) && empty($template)) {
				$replace_str = array('\'', '"', '#', '&', '+', '<', '>', ',');
				...
				if ($result['ResultCode'] == '100') {
					...
					foreach ($orderids as $orderid) {
						...
						if ($orderinfo['status'] == 1 || $orderinfo['status'] == 0 && $orderinfo['paytype'] == 3) {
							$data = array('express' => trim($esheet_item['express']), 'expresssn' => trim($result['Order']['LogisticCode']), 'expresscom' => trim($esheet_item['name']), 'sendtime' => time(), 'refundstate' => 0, 'print_template' => $result['PrintTemplate']);
				...
				pdo_query('UPDATE ' . tablename('ewei_shop_order_goods') . ' SET `esheetprintnum` = `esheetprintnum` + 1 WHERE id in (' . $_GPC['printarr']['ordergoodid'] . ')');
				exit(json_encode(array('result' => 'success', 'PrintTemplate' => $template, 'issend' => $esheet_item['issend'])));
			}
			else {
				exit(json_encode(array('result' => 'error', 'resp' => '错误，请重试')));
			}
		}
	}
}
