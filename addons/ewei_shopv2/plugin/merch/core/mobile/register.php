	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($reg)) {
				...
				$regid = pdo_insertid();
				$user = array('img'=>$_GPC['img'],'chepai'=>$_GPC['chepai'],'uniacid' => $_W['uniacid'], 'merchname' => trim($data['merchname']), 'salecate' => trim($data['salecate']), 'realname' => $data['realname'], 'mobile' => trim($data['mobile']), 'status' => -1, 'regid' => $regid, 'upass' => $data['upass'], 'openid' => $_W['openid'], 'uname' => $uname);
				pdo_insert('ewei_shop_merch_user', $user);
			}
			else {
				pdo_update('ewei_shop_merch_reg', $data, array('id' => $reg['id']));
				$user = array('img'=>$_GPC['img'],'chepai'=>$_GPC['chepai'],'uniacid' => $_W['uniacid'], 'merchname' => trim($data['merchname']), 'salecate' => trim($data['salecate']), 'realname' => $data['realname'], 'mobile' => trim($data['mobile']), 'status' => -1, 'regid' => $reg['id'], 'uname' => $uname, 'openid' => $_W['openid'], 'upass' => $data['upass']);
