...
class Detail_EweiShopV2Page extends MerchWebPage
{
	...
	public function lingqu(){
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if($item['merchid']!=0){
			$this->message('此订单已被他人领取！', webUrl('order/list/status10'), 'error');
		}else{
			pdo_update('ewei_shop_order', array('merchid' =>$_W['merchid'],'status'=>0), array('id' => $id, 'uniacid' => $_W['uniacid']));
			$this->message('领取成功！', webUrl('order/detail/status10'), 'success');
		}
