...
class Index_EweiShopV2Page extends PluginWebPage
{
	...
	public function post_new()
	{
		...
		if ($_W['ispost']) {
			...
			$this->replyRule($data['keyword_pick']);
	...
	public function replyRule($keyword)
	{
		global $_W;
		...
		$id = pdo_fetchcolumn('select id from ' . tablename('rule_keyword') . (' where uniacid = ' . $_W['uniacid'] . ' and module = \'ewei_shopv2\' and rid = ' . $rid . ' '));
		...
		if (empty($id)) {
			...
			pdo_update('rule_keyword', array('content' => $keyword), array('rid' => $rid));
		}
	}
