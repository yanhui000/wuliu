define(['core', 'tpl', 'biz/plugin/diyform'], function (core, tpl, diyform) {
    ...
    modal.init = function (params) {
        ...
        $('.date-btn-cancel').click(function () {
            $('.date-picker').fadeOut()
            var nowtime = $('.predicttime').attr('data-date');
            setTimeout(function(){
                $('.day').each(function(){
                    if($(this).attr('data-id') == nowtime){
        ...
        $('.date-btn-confirm').click(function () {
            $('.day').each(function(){
                    if($(this).hasClass('active')){
                        var showyear = $(this).attr('data-id').substr(0,4);
                        var showmonth = $(this).attr('data-id').substr(4,2);
                        var showday = $(this).attr('data-id').substr(6,2);
                        var showtime = showyear+'.'+showmonth+'.'+showday;
                        $('.predicttime').html(showtime).attr('data-date',$(this).attr('data-id'));
                    }
            })
        ...
        $('.select-showtime').click(function () {
            var dates = new Date();
    ...
    modal.caculate = function () {
        ...
        core.json('cycelbuy/order/create/caculate', {
            ...
            if (getjson.status == 1) {
                if (modal.params.iscarry) {
                    ...
                    $('.dispatchprice').html(core.number_format(getjson.result.price, 2)+'('+getjson.result.cycelbuy_num+'期运费)')
                }
    ...
    modal.submit = function (obj, token) {
        ...
        var data = {
            ...
            'predicttime':$(".predicttime").attr('data-date')
        };
        FoxUI.loader.show('mini');
        core.json('cycelbuy/order/create/submit', data, function (ret) {
            ...
            $('#areas').cityPicker({
                ...
                onClose: function (self) {
                    ...
                    if (modal.params.new_area && modal.params.address_street) {
                        ...
                        street.cityPicker({title: '请选择所在街道', street: 1, data: data,onClose: function (self) {
