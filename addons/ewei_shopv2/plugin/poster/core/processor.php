...
class PosterProcessor extends PluginProcessor
{
	...
	private function responseText($obj)
	{
		...
		if ($resp['code'] == 302 || $resp['code'] == 301) {
			...
			$resp = ihttp_request('https:' . $url, array('openid' => $obj->message['from'], 'content' => urlencode($obj->message['content'])), array(), $timeout);
			if ($resp['code'] == 302 || $resp['code'] == 301) {
				$resp = ihttp_request('http:' . $url, array('openid' => $obj->message['from'], 'content' => urlencode($obj->message['content'])), array(), $timeout);
			}
		}
