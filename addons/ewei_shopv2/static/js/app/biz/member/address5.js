define(['core', 'tpl'], function(core, tpl) {
    ...
    modal.initPost = function(params) {
        var reqParams = ['foxui.picker1'];
        if (params.new_area) {
            reqParams = ['foxui.picker1', 'foxui.citydatanew']
        }
        require(reqParams, function() {
            $('#areas5').cityPicker({
                ...
                onClose: function(self) {
                    var datavalue = $('#areas5').attr('data-value');
        ...
        $(document).on('click', '#btn-address', function() {
            wx.openAddress({
                success: function(res) {
                    ...
                    $('#areas5').val(res.provinceName + " " + res.cityName + " " + res.countryName);
                    ...
                    var province_code = 0,
                        city_code = 0,
                        county_code = 0;
                    var xmlfile = '../addons/ewei_shopv2/static/js/dist/area/AreaNew1.xml';
                    ...
                    $("#areas5").attr("data-value", province_code + " " + city_code + " " + county_code)
                }
            })
        });
        $(document).on('click', '#btn-submit', function() {
            ...
            var jingwai = /(境外地区)+/.test($('#areas5').val());
            var taiwan = /(台湾)+/.test($('#areas5').val());
            var aomen = /(澳门)+/.test($('#areas5').val());
            var xianggang = /(香港)+/.test($('#areas5').val());
            ...
            window.editAddressData = {
                ...
                datavalue: $('#areas5').attr('data-value'),
