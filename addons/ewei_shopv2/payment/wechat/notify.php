...
class EweiShopWechatPay
{
	...
	public function order()
	{
		...
		if (!empty($log) && ($log['status'] == '0' || $ispeerpay) && ($log['fee'] == $this->total_fee || $ispeerpay)) {
			if ($count_ordersn == 2) {
				pdo_update('ewei_shop_order', array('tradepaytype' => 21, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0), array('ordersn_trade' => $log['tid'], 'uniacid' => $log['uniacid']));
			}
			else {
