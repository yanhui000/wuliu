...
<style>
	...
	.dingtop{font-size: .65rem; padding: .6rem; color: #000; border-bottom:1px solid #ebebeb;}
	.dingtop img{ margin-right: .4rem;width: 1rem; vertical-align: middle; }
	.dingtop .sp{ color: #999; font-size: .6rem; margin-left: 1rem}
	.dingtop span{ vertical-align: middle; }
	.shi{ vertical-align: middle; margin:.4rem 0; }
	.tow{ background-color: #F2F2F2; padding:.1rem .2rem }
	.golist{ overflow: hidden;padding: .4rem .6rem; }
	.golist1{ float: left; width: 12%; margin-right: .6rem }
	.golist1 img{ width: 2rem; height: 2rem }
	.golist2{ float: left; color: #000; width: 73%;}
	.golist3{float: right; width: 10%; line-height: 2rem; text-align: right; }
	.golist3 img{ width: 1rem }
	.towp{font-size: .75rem;white-space: nowrap;text-overflow: ellipsis;overflow: hidden;}
...
<div class='fui-page order-list-page '>
    ...
    <div id="tab" class="fui-tab fui-tab-danger">
		...
		<a data-tab="tab" class="external <?php  if($_GPC['status']=='10') { ?>active<?php  } ?>"  data-status='10'>待确认</a>
        <a data-tab="tab0" class="external <?php  if($_GPC['status']=='0') { ?>active<?php  } ?>"  data-status='0'>待支付</a>

        <a data-tab="tab1" class="external <?php  if($_GPC['status']=='1') { ?>active<?php  } ?>"  data-status='1'>待发货</a>

        <a data-tab="tab2" class="external <?php  if($_GPC['status']=='2') { ?>active<?php  } ?>"  data-status='2'>运输中</a>
    ...
    <div class='fui-content navbar order-list' >
        ...
        <div class='fui-content-inner'>
            ...
            <div class='content-empty' style='display:none;position: absolute; left: 50%; top: 50%;transform: translate(-50%, -50%); margin-top: 0'>

                <!--<i class='icon icon-lights'></i><br/>暂时没有任何订单<br/><a href="<?php  echo mobileUrl()?>" class='btn btn-default-o external'>到处逛逛</a>-->

                <img src="<?php echo EWEI_SHOPV2_STATIC;?>images/nolist.png" style="width: 6rem;margin-bottom: .5rem;"><br/><p style="color: #999;font-size: .75rem">暂无相关订单</p><br/><!-- <a href="<?php  echo mobileUrl()?>" class='btn btn-sm btn-danger-o external' style="border-radius: 100px;height: 1.9rem;line-height:1.9rem;width:  7rem;font-size: .75rem">去首页逛逛吧   </a> -->
    ...
    <script id='tpl_order_index_list' type='text/html'>
        ...
        <div class='fui-list-group order-item' data-orderid="<%order.id%>" data-verifycode="<%order.verifycode%>">
				...
				<div class="golist">
                    <div class="golist1">
                        <img src="<%order.img%>" class="" style="width: 2rem; height: 2rem">
                    </div>
                    <div class="golist2">
                        <div class="towp">
                            <span><%order.realname%></span>
                            <span class="tow"><%order.chepai%></span>
                          
                        </div>
                       
                    </div>
                    <div class="golist3">
                        <a href="tel:<%order.mobile%>"><img src="../addons/ewei_shopv2/template/mobile/default/static/images/tel.png"></a>
                    </div>
                </div>
				<%/if%>
                <div class='fui-list-group-title lineblock'>
                    ...
                    <%else%>
					<%if order.status!=10 %>
                    <span class='status noremark'>实付: <span class='text-danger'>&yen; <span class="bigprice"><%order.price%></span></span></span>
					<%else%>
					<span class='status noremark'>期望运费: <span class='text-danger'>&yen; <span class="bigprice"><%order.price%></span></span></span>
					<%/if%>
                    <%/if%>
 ...
 <%if order.status!=10 && order.addressid>0 && order.iscycelbuy == 0 %>
